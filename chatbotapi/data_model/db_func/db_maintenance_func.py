...
def getAllMaintenanceByIdSubscribe(id_subcribe_product):
    subscribe_product = session.query(Maintenance).join(User, User.id_user == Maintenance.id_user).filter(Maintenance.id_subcribe_product == id_subcribe_product and Maintenance.status_code == 1).all()
    session.commit()
    
    return subscribe_product

def getSpecificMaintenance(id_subcribe_product, request_date):
    subscribe_product = session.query(Maintenance,User).join(User, User.id_user == Maintenance.id_user).filter(Maintenance.id_subcribe_product == id_subcribe_product and Maintenance.request_date == request_date).first()
    session.commit()
